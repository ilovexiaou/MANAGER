    ...
    <form action="<%=basePath%>role/updateRole.do"  method="post"  accept-charset="utf-8">
         ...
         <div style="margin:10px 0;font-family: Arial;border-radius: 10px; border-style:solid; border-color: #E5E5E5;border-width: 1px;">
            <table style="width: 100%;font-size: 12px;"  border="0" cellspacing="5px;" cellpadding="0">
                <tr>
                    <td >角色名:</td>
                    <td> <input  name="name" value="${role.name} " class="easyui-textbox" required="required" ></td>
                    <td>是否启用:</td>
                    <td>
                        <span class="radioSpan" >
                            <input name="flag" type="radio" name="flag" value="1">启用</input>
                ...
                <tr>
                    <td>角色描述:</td>
                    <td colspan="3">
                        <input name="coment" value = "${role.coment}" class="easyui-textbox" data-options="multiline:true"  style="width:90%;height:80px">
                    </td>
                </tr>
            </table>
        </div>
        ...
        success: function(data){
            ...
            $(document).ready(function(){
                ...
                $.ajax({ 
                    ...
                    traditional: true,
                    async:false,
                    success: function(treeDate2){
                    	var dataObj2 = eval(treeDate2);
                    	for(var i = 0;i<array.length;i++){
                    		for(var j = 0;j<dataObj2.length;j++){
                    			if(array[i].id==dataObj2[j].id){
                    				ccids +=  dataObj2[j].id+","
                    				if(dataObj2[j].level=='3'){
